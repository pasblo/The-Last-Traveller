...
def ordenardbniveles(user, nivel, forma = "puntuacion"): #Primero el mejor y asi en orden
	...
	for i in LALISTA:
		sql = """
		INSERT INTO %s(puntuacion, matados, llegado, muertes, tiempo)
		VALUES (?, ?, ?, ?, ?)""" %("Lv" + str(nivel))


		consulta.execute(sql, (i[0], i[1], i[2], i[3], i[4]))
...
def calcularnivelact(user):
	...
	while cont < 31:
		...
		for datos in total:
			...
			if len(total) != 0 and datos[2] > 99 and encontrado == False:
				nivel_act += 1
				encontrado = True
